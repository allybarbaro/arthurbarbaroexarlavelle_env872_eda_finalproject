...
shape.and.race <- merge(carteret.shape, carteret.race.percent, by = "TRACTCE")

shape.and.race <- st_transform(shape.and.race, crs = '+proj=longlat +datum=WGS84')

leaflet() %>%
  addTiles() %>%
  setView(lng = -076.35, lat = 34.88, zoom = 14) %>%
  addPolygons(data = shape.and.race, color = "blue", stroke = 1, opacity = 0.8)
